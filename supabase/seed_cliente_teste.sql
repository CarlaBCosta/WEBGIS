-- Cria o cliente "teste" com a mesma estrutura de grupos/camadas do
-- catálogo de referência (PRD-webgis.md), para testar o portal público em
-- /cliente/teste. Rode no SQL Editor do Supabase (requer que
-- supabase/setup_completo.sql já tenha sido aplicado antes, para as
-- tabelas/policies existirem).
--
-- Isso só cria os METADADOS (cliente, grupos, camadas e seus estilos).
-- Os arquivos .geojson com a geometria real ainda precisam ser enviados
-- depois, via painel /admin/clientes/teste/upload (cada layer_key abaixo
-- corresponde ao nome esperado do arquivo, ex: Usina_Principal_teste.geojson).

with c as (
    insert into clients (slug, name, map_center_lat, map_center_lng, map_zoom, zoom_to_layer)
    values ('teste', 'Cliente Teste', -21.90, -48.67, 11, 'Area_de_Influencias_Direta_teste')
